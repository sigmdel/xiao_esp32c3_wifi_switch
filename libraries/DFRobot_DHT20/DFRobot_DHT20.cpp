...
TempAndHumidity_t DFRobot_DHT20::getTempAndHumidity(void) {
  uint8_t readCMD[3]={0xac,0x33,0x00};
  uint8_t data[6] = {0};
  int retries = 10;
  TempAndHumidity_t values;
  ...
  DBG(rawData);
  DBG((temp&0xf)<<16);
  values.temperature = (float)rawData/5242 -50;
  DBG(values.temperature)
  ...
  DBG(rawData);
  DBG(temp<<12);
  values.humidity = (float)rawData/0x100000;
  DBG(values.humidity)
