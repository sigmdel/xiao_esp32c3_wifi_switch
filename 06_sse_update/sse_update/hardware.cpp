...
void setLed(int value) {
  ...
  updateDomoticzSwitch(SWITCH_IDX, value);    // update Domoticz
...
void readTemp(void) {
  if (millis() - temptime > SENSOR_DELAY) {
    ...
    temptime = millis();
    updateDomoticzTemperatureHumiditySensor(TEMP_HUMI_IDX, tah.temperature, 100*tah.humidity);  // update Domoticz
...
void readLight() {
  if (millis() - lighttime >= SENSOR_DELAY) {
    ...
    Light = String(value);
    lighttime = millis();
    updateDomoticzLightSensor(LUX_IDX, value);     // update Domoticz
